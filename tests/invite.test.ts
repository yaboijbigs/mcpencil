...
describe("room invitations", () => {
  ...
  it("delimits the exact agent URL and supplies the complete WebMCP play contract", () => {
    ...
    expect(agent.text).toContain("submit visually supported guesses from that snapshot while the human keeps drawing");
    expect(agent.text).toContain("New strokes or a newer canvasVersion do not invalidate the observed snapshot");
    expect(agent.text).toContain("finish the current guess attempt without rechecking or waiting for the canvas to stop changing");
    expect(agent.text).toContain("Incorporate new strokes in the next observation cycle");
    ...
    expect(agent.text).toContain("distinct, visually supported refinements");
    expect(agent.text).toContain("if none are plausible, briefly wait again");
    expect(agent.text).toContain("Discard the old picture when the round or artist changes");
    expect(agent.text).toContain("stop guessing when the phase or role no longer permits it");
